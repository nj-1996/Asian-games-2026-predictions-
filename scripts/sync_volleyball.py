...
def main():
    ...
    os.makedirs("data/volleyball", exist_ok=True)

    if len(all_men) > 0:
        with open("data/volleyball/tracker_men.json", "w", encoding="utf-8") as f:
            json.dump({"sport": "Volleyball (Men)", "matches": all_men}, f, indent=2, ensure_ascii=False)
        print(f"Saved {len(all_men)} Men's volleyball fixtures.")
    else:
        print("⚠️ No Men's fixtures returned from API. Preserving existing tracker_men.json.")

    if len(all_women) > 0:
        with open("data/volleyball/tracker_women.json", "w", encoding="utf-8") as f:
            json.dump({"sport": "Volleyball (Women)", "matches": all_women}, f, indent=2, ensure_ascii=False)
